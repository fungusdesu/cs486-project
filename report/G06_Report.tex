...
Agent work was performed primarily through Codex and Antigravity, using the following models:
...
\subsubsection{Solution}
Created \texttt{.codex/skills/db-design-pipeline/SKILL.md}:
...
\subsubsection{Problem}
Baseline agent assumed a single tool. The improved project standardizes on Codex and Antigravity.
...
\hline
Tool support & 1 baseline tool & 2 primary tools (Codex + Antigravity) \\
\hline
...
\hline
\texttt{AGENTS.md} & Global rules, workflow order, design principles, DBMS choice & Codex and Antigravity \\
\hline
...
\hline
\texttt{SKILL.md} & Detailed per-step templates, prerequisites, validation criteria & Codex (native), Antigravity (via AGENTS.md pointer) \\
\hline
...
\texttt{AGENTS.md} is the primary configuration file used by Codex and Antigravity at the start of every session.

\subsection{Header and Tool Compatibility}

\begin{lstlisting}
> This file is the shared instruction set for every
> agent tool used on this project (Codex,
> Antigravity, and others that read
> AGENTS.md natively).
\end{lstlisting}
...
Codex loads the canonical skill natively, and Antigravity reaches it through the \texttt{AGENTS.md} pointer.
...
The \texttt{SKILL.md} file at \texttt{.codex/skills/db-design-pipeline/} provides detailed per-step enforcement:
...
\hline
Antigravity & Reads \texttt{AGENTS.md} & Via AGENTS.md pointer to \texttt{.codex/skills/} \\
\hline
Codex & Reads \texttt{AGENTS.md} & Native skill loader at \texttt{.codex/skills/} \\
\hline
\end{tabular}
\caption{Tool support matrix}
\end{table}

All supported tools converge on the same rules through \texttt{AGENTS.md} and the canonical skill pointer.
...
Every constraint traces directly to a teacher guideline from the reference project. The system centers on the two tools used by the group, Codex and Antigravity, without rule duplication, and enables auditable, cost-efficient database design work. All improvements are documented, reversible, and traceable to specific design weaknesses in the baseline agent.
...
\hline
.codex/skills/db-design-pipeline/SKILL.md & Detailed per-step templates, prerequisites, validation criteria \\
\hline
