...
\subsection{Improvement 1: Extended Pipeline (From 2 Steps to 7 Steps)}
...
\subsubsection{Solution}
Created \texttt{.opencode/skills/db-design-pipeline/SKILL.md}:
...
\subsubsection{Problem}
Baseline agent assumed single tool (OpenCode). No guidance for other tools (Antigravity, Codex CLI).
...
\hline
\texttt{SKILL.md} & Detailed per-step templates, prerequisites, validation criteria & OpenCode (native), Antigravity/Codex (via AGENTS.md pointer) \\
\hline
...
> OpenCode, Codex CLI, and others that read
> AGENTS.md natively).
\end{lstlisting}
...
Antigravity and Codex CLI reach the canonical skill path directly via the \texttt{AGENTS.md} pointer.
...
Every constraint traces directly to a teacher guideline from the reference project. The system supports three agent tools (Antigravity, OpenCode, Codex CLI) without rule duplication, and enables auditable, cost-efficient database design work. All improvements are documented, reversible, and traceable to specific design weaknesses in the baseline agent.
