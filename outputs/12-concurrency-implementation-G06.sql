...
(
    ...
    FROM Maintenance AS m
    	INNER JOIN dbo.MaintenanceSession AS ms ON ms.maintenance_id = m.maintenance_id
    	INNER JOIN lookup_table.MaintenanceImpactLevel AS mil ON mil.maintenance_impact_level_id = ms.maintenance_impact_level_id
    	INNER JOIN lookup_table.MaintenanceStatus AS mts ON mts.maintenance_status_id = m.maintenance_status_id
    	INNER JOIN dbo.BookingRequest AS br ON br.space_id = m.space_id
    	INNER JOIN dbo.Reservation AS r ON r.booking_request_id = br.booking_request_id
    	INNER JOIN lookup_table.ReservationStatus AS rs ON rs.reservation_status_id = r.reservation_status_id
    WHERE (
...
(
    SELECT
        br.space_id,
        s.space_name,
		SUM((DATEDIFF(minute, rse.actual_start_time, rse.actual_end_time)) / 60.0) AS total_reservation_hours
    FROM ReservationSession AS rse
    	INNER JOIN dbo.Reservation AS r ON r.reservation_id = rse.reservation_id
    	INNER JOIN dbo.BookingRequest AS br ON br.booking_request_id = r.booking_request_id
    	INNER JOIN dbo.Space AS s ON s.space_id = br.space_id
    WHERE (
		rse.actual_end_time IS NOT NULL
		AND (@start_time IS NULL OR rse.actual_start_time >= @start_time)
		AND (@end_time   IS NULL OR rse.actual_end_time   <= @end_time)
	)
    GROUP BY br.space_id, s.space_name
);
GO
