...
### Claude Code
Does **not** read `AGENTS.md` automatically — it looks for `CLAUDE.md`
specifically. This repo's `CLAUDE.md` is a one-line pointer back to
`AGENTS.md`/`MEMORY.md` so the rules aren't duplicated.
Skill: `.claude/skills/db-design-pipeline/SKILL.md` (synced copy — do not
edit directly, edit the canonical one in `.opencode/` and run
...
identity or system file under `.openclaw/`), add a short pointer there the
same way `CLAUDE.md` does, rather than re-stating the rules.

### Claude.ai / Claude in a browser chat (no filesystem access)
There's no repo for Claude to read on its own. Paste the contents of
`AGENTS.md` and `MEMORY.md` (or upload them as files) at the start of the
conversation, then use the prompts in `PROMPTS.md`.
...
2. If no, add a one-line pointer file at the location that tool expects
   (mirroring `CLAUDE.md`), pointing back to `AGENTS.md` + `MEMORY.md`.
...
This refreshes `.claude/skills/` and `.openclaw/skills/`. Antigravity and
